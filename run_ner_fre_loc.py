...
def main():
    ...
    output_dir = '_'.join(['./saver/',args.data_dir.split('/')[-1], args.bert_model, 'frequency', str(args.max_seq_length), str(args.learning_rate), str(args.bert_lr), str(args.warmup_proportion),str(args.train_batch_size),str(int(args.num_train_epochs)), str(args.seed) ])
    if args.use_crf:
        output_dir+='_crf'
    if args.use_rnn:
        output_dir += '_rnn'
    ...
    if args.do_train:
        train_features, difficulty_score = convert_examples_to_features(
            train_examples, label_list, args.max_seq_length, tokenizer, True, curriculum='frequency')
